...
<div class="card border-0 shadow-sm">
    ...
    <div class="card-body">
        <form method="POST" action="{{ isset($monitoring) ? route('monitoring.update', $monitoring->id) : route('monitoring.store') }}">
            @csrf
            @if (isset($monitoring))
                @method('PUT')
            ...
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <select name="kategori" class="form-select" required>
                        <option value="" disabled hidden {{ $selectedKategori === '' ? 'selected' : '' }}></option>
                        <option value="HF Nelayan" {{ $selectedKategori === 'HF Nelayan' ? 'selected' : '' }}>HF Nelayan</option>
                        <option value="HF Rutin" {{ $selectedKategori === 'HF Rutin' ? 'selected' : '' }}>HF Rutin</option>
                        <option value="MF" {{ $selectedKategori === 'MF' ? 'selected' : '' }}>HF Medium Frequency</option>
                    </select>
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" step="0.001" name="frekuensi_khz" class="form-control" value="{{ old('frekuensi_khz', $monitoring->frekuensi_khz ?? '') }}">
                </div>
                ...
                @if (isset($monitoring))
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="datetime-local" id="editMulaiPengamatanInput" name="mulai_pengamatan" class="form-control" value="{{ old('mulai_pengamatan', $mulaiPengamatanDefault) }}">
                    </div>
                    <div class="col-md-6">
                        <label class="form-label">Selesai Pengamatan <span class="text-danger">*</span></label>
                        <input type="time" id="editSelesaiPengamatanInput" name="selesai_pengamatan_waktu" class="form-control" step="60" value="{{ old('selesai_pengamatan_waktu', $jamAkhirStored) }}">
                    </div>
                @else
                    <div class="col-md-4">
                        <label class="form-label">Tanggal (1-31)</label>
                        <select name="tanggal" class="form-select">
                            <option value="" disabled hidden {{ old('tanggal', $monitoring->tanggal ?? '') === '' ? 'selected' : '' }}></option>
                            @for ($day = 1; $day <= 31; $day++)
                                <option value="{{ $day }}" {{ (string) old('tanggal', $monitoring->tanggal ?? '') === (string) $day ? 'selected' : '' }}>{{ str_pad((string) $day, 2, '0', STR_PAD_LEFT) }}</option>
                            @endfor
                        </select>
                    </div>
                    <div class="col-md-4">
                        ...
                        <select name="bulan" class="form-select">
                            <option value="" disabled hidden {{ old('bulan', $monitoring->bulan ?? '') === '' ? 'selected' : '' }}></option>
                            @php
                                $monthNames = [
                                    1 => 'Januari',
                                    2 => 'Februari',
                            ...
                            @foreach ($monthNames as $monthNumber => $monthLabel)
                                <option value="{{ $monthNumber }}" {{ (string) old('bulan', $monitoring->bulan ?? '') === (string) $monthNumber ? 'selected' : '' }}>{{ str_pad((string) $monthNumber, 2, '0', STR_PAD_LEFT) }} - {{ $monthLabel }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        ...
                        <select name="tahun" class="form-select">
                            <option value="" disabled hidden {{ old('tahun', $monitoring->tahun ?? '') === '' ? 'selected' : '' }}></option>
                            @for ($year = 2024; $year <= 2026; $year++)
                                <option value="{{ $year }}" {{ (string) old('tahun', $monitoring->tahun ?? '') === (string) $year ? 'selected' : '' }}>{{ $year }}</option>
                            @endfor
                        </select>
                    </div>
                    ...
                    <div class="col-md-6">
                        <label class="form-label">Mulai Pengamatan <span class="text-danger">*</span></label>
                        @php
                            $jamMulaiRaw = (string) old('jam_mulai', $monitoring->jam_mulai ?? '');
                    ...
                    <div class="col-md-6">
                        <label class="form-label">Selesai Pengamatan <span class="text-danger">*</span></label>
                        @php
                            $jamAkhirRaw = (string) old('jam_akhir', $monitoring->jam_akhir ?? '');
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" step="0.01" name="kuat_medan_dbuvm" class="form-control" value="{{ old('kuat_medan_dbuvm', $monitoring->kuat_medan_dbuvm ?? '') }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label">Identifikasi <span class="text-danger">*</span></label>
                    <input type="text" name="identifikasi" class="form-control" value="{{ old('identifikasi', $monitoring->identifikasi ?? '') }}" placeholder="Isi UNKNOWN jika tidak teridentifikasi">
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="lebar_band" class="form-control" value="{{ old('lebar_band', $monitoring->lebar_band ?? '') }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label">Kelas Emisi <span class="text-danger">*</span></label>
                    <input type="text" name="kelas_emisi" class="form-control" value="{{ old('kelas_emisi', $monitoring->kelas_emisi ?? '') }}">
                </div>

                <div class="col-md-2">
                    <label class="form-label">Long (0-180)</label>
                    <input type="text" name="longitude_derajat" class="form-control" value="{{ old('longitude_derajat', $monitoring->longitude_derajat ?? '') }}">
                </div>
                <div class="col-md-2">
                    <label class="form-label">E atau W</label>
                    <input type="text" name="longitude_arah" class="form-control" value="{{ old('longitude_arah', $monitoring->longitude_arah ?? '') }}">
                </div>
                <div class="col-md-2">
                    <label class="form-label">Long (0-59)</label>
                    <input type="text" name="longitude_menit" class="form-control" value="{{ old('longitude_menit', $monitoring->longitude_menit ?? '') }}">
                </div>
                <div class="col-md-2">
                    <label class="form-label">Lat (0-90)</label>
                    <input type="text" name="latitude_derajat" class="form-control" value="{{ old('latitude_derajat', $monitoring->latitude_derajat ?? '') }}">
                </div>
                <div class="col-md-2">
                    <label class="form-label">N atau S</label>
                    <input type="text" name="latitude_arah" class="form-control" value="{{ old('latitude_arah', $monitoring->latitude_arah ?? '') }}">
                </div>
                <div class="col-md-2">
                    <label class="form-label">Lat (0-59)</label>
                    <input type="text" name="latitude_menit" class="form-control" value="{{ old('latitude_menit', $monitoring->latitude_menit ?? '') }}">
                </div>

                <div class="col-md-4">
                    <label class="form-label">North Bearing</label>
                    <input type="text" name="north_bearing" class="form-control" value="{{ old('north_bearing', $monitoring->north_bearing ?? '') }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label">Akurasi</label>
                    <input type="text" name="akurasi" class="form-control" value="{{ old('akurasi', $monitoring->akurasi ?? '') }}">
                </div>
                <div class="col-md-4">
                    <label class="form-label">Tidak Sesuai RR</label>
                    <input type="text" name="tidak_sesuai_rr" class="form-control" value="{{ old('tidak_sesuai_rr', $monitoring->tidak_sesuai_rr ?? '') }}">
                </div>

                <div class="col-12">
                    <label class="form-label">Perkiraan Lokasi Sumber Pancaran</label>
                    <input type="text" name="perkiraan_lokasi_sumber_pancaran" class="form-control" value="{{ old('perkiraan_lokasi_sumber_pancaran', $monitoring->perkiraan_lokasi_sumber_pancaran ?? '') }}">
                </div>

                <div class="col-12">
                    <label class="form-label">Informasi Tambahan</label>
                    <textarea name="informasi_tambahan" class="form-control" rows="3">{{ old('informasi_tambahan', $monitoring->informasi_tambahan ?? '') }}</textarea>
                </div>
            </div>
