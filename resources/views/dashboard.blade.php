    ...
    <div class="row mb-3">
        <div class="col-12 d-flex justify-content-end">
            <div class="d-flex align-items-center gap-2 bg-white px-3 py-2 rounded-pill shadow-sm" style="border: 1px solid #f1f5f9;">
                ...
                <select name="user_id" class="form-select form-select-sm border-0 fw-bold text-dark cursor-pointer bg-slate-50" style="font-size: 0.8rem; width: auto; min-width: 150px; border-radius: 12px; padding-left: 10px;" onchange="window.refreshDashboard(this.value)">
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover align-middle mb-0" style="font-size: 0.73rem;">
                                ...
                                <tbody class="border-0">
                                            ...
                                            <td class="px-3">
                                                <div class="d-flex align-items-center gap-2">
                                                    @if($monitoring->user && $monitoring->user->profile_photo)
                                                        <img src="{{ asset('storage/' . $monitoring->user->profile_photo) }}" class="avatar-circle-sm">
                                                    @else
                                                        <div class="avatar-circle-sm d-flex align-items-center justify-content-center bg-slate-100 text-slate-400" style="font-size: 0.6rem; font-weight: 800;">
                                                            {{ strtoupper(substr($monitoring->user->name ?? '?', 0, 1)) }}
                                                        </div>
                                                    @endif
                                                    <span class="fw-bold text-slate-700" style="font-size: 0.65rem;">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="6" class="text-center py-4">Belum ada data monitoring</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
...
@section('scripts')
    ...
    <script>
        (function() {
            // FUNGSI DASHBOARD TURBO (AJAX)
            window.refreshDashboard = async (userId) => {
                const wrapper = document.getElementById('dashboard-content-wrapper');
                if (!wrapper) return;

                const url = `{{ route('dashboard') }}?user_id=${userId}`;
                ...
                try {
                    ...
                    // Ekstrak hanya bagian content-wrapper dari HTML yang diterima
                    const tempDiv = document.createElement('div');
                    tempDiv.innerHTML = html;
                    const newContent = tempDiv.querySelector('#dashboard-content-wrapper');
                    
                    if (newContent) {
                        wrapper.innerHTML = newContent.innerHTML;
                        wrapper.style.opacity = '1';
                        
                        // Re-inisialisasi Chart
                        window.initDashboardCharts();
                        
                        // Update URL (Hide Params)
                        window.history.replaceState({}, '', window.location.pathname);
                    }
                } catch (error) {
                    console.error('Turbo Load Failed:', error);
                    wrapper.style.opacity = '1';
                }
            };
            ...
            window.initDashboardCharts = function() {
                ...
                if (window.balmonBarChart) window.balmonBarChart.destroy();
                if (window.balmonMonthlyChart) window.balmonMonthlyChart.destroy();

                let barData = { labels: [], datasets: { mf: [], rutin: [], nelayan: [] } };
                let monthlyData = { labels: [], datasets: { mf: [], rutin: [], nelayan: [] } };
                ...
                window.balmonBarChart = new Chart(barCtx, {
                    ...
                    data: {
                        ...
                        datasets: [
                            { label: 'MF', data: barData.datasets.mf, backgroundColor: '#3b82f6', borderRadius: 5 },
                            { label: 'HF Rutin', data: barData.datasets.rutin, backgroundColor: '#10b981', borderRadius: 5 },
                            { label: 'HF Nelayan', data: barData.datasets.nelayan, backgroundColor: '#f43f5e', borderRadius: 5 }
                        ]
                    },
                ...
                window.balmonMonthlyChart = new Chart(monthlyCtx, {
                    ...
                    data: {
                        ...
                        datasets: [
                            { label: 'MF', data: monthlyData.datasets.mf, backgroundColor: '#3b82f6', borderRadius: 5 },
                            { label: 'HF Rutin', data: monthlyData.datasets.rutin, backgroundColor: '#10b981', borderRadius: 5 },
                            { label: 'HF Nelayan', data: monthlyData.datasets.nelayan, backgroundColor: '#f43f5e', borderRadius: 5 }
                        ]
                    },
                    options: chartDefaults
                });
